...
namespace BackOffice\CleanBundle\Manager;
...
class CleanManager
{
    ...
    public $fatalErrorMet = false; // true si une erreur (fatale) a été rencontrée
    ...
    public function addError($libelle, $module = '', $action = '')
    {
        $this->fatalErrorMet = true;
        $this->addLog(Log::NIVEAU_ERREUR, $libelle, $module, $action);
    }
    ...
    public function addLog($niveau, $libelle, $module = '', $action = '')
    {
    ...
    public function getDestMailErreur()
    {
    ...
    private function sendMail($mailObject)
    {
        $logs = null;
    ...
    public function sendMailProcessError($log, $tabLogCompt = array(), $titre = '')
    {
        $headers  = 'MIME-Version: 1.0' . "\r\n";
        $headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";
        $headers .= 'From: BFI <noreply@banque-fiducial.fr>' . "\r\n";
